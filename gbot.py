...
def shopReminder():
    ...
    while 1:
        s.enterabs(shop_reminder_time_secs(), 1, send_shopping_message)
...
def shop_reminder_time_secs():
    ...
    inc = 0
    if now.hour >= 18:
        # add one day if after 6pm
        inc = 1
        # add one more day if it's Saturday
        if now.weekday == 5:
            inc == 2
    # add one day if it's Sunday before 6
    if now.weekday == 6:
        inc = 1

    # time in seconds since epoch
    remind_time = datetime(now.year, now.month, now.day + inc, 18).timestamp()
...
broadcast_message("Sorry, went out of power :( Please push /start. Love, your GoutmetBot.")
